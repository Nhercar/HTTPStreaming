...
std::string buildHttpResponse(const HTTPResponse& resp){
    ...
    if (!resp.body.empty() && headers.find("Content-Length") == headers.end() && headers["Connection"] != "keep-alive"){ //Si no se ha añadido el argumento Content-Length se añade automáticamente
